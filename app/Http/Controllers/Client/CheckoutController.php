...
class CheckoutController extends Controller
{
    ...
    {
        // 1) Áp dụng coupon (nếu bấm nút)
        if ($request->has('apply_coupon')) {
            return $this->apply($request);
        }

        // 2) Checkbox: chỉ true khi thực sự được tick
        $shipFlag = ($request->filled('ship_to_different') && $request->input('ship_to_different') != '0') ? '1' : '0';

        // Snapshot toàn bộ payload và thay ship_to_different = $shipFlag
        $data = $request->all();
        $data['ship_to_different'] = $shipFlag;
        \Log::info('checkout payload (normalized)', $data);

        // 3) Validate theo nhánh
        // 2) Validate theo nhánh
        if ($shipFlag === '1') {
            $request->validate([
        ...
        try {
            ...
            if ($recipientEmail && filter_var($recipientEmail, FILTER_VALIDATE_EMAIL)) {
                DB::afterCommit(function () use ($order, $recipientEmail) {
                    try {
                        Mail::to($recipientEmail)->send(new OrderConfirmationMail($order));
                        \Log::info('Order mail sent (COD)', ['order_id' => $order->id, 'to' => $recipientEmail]);
                    } catch (\Throwable $e) {
                        \Log::error('Send order mail failed (COD): ' . $e->getMessage());
                    }
                });
            } else {
            ...
            try {
                ...
                if ($recipientEmail && filter_var($recipientEmail, FILTER_VALIDATE_EMAIL)) {
                    DB::afterCommit(function () use ($order, $recipientEmail) {
                        try {
                            Mail::to($recipientEmail)->send(new OrderConfirmationMail($order));
                            \Log::info('Order mail sent (VNPAY)', ['order_id' => $order->id, 'to' => $recipientEmail]);
                        } catch (\Throwable $e) {
                            \Log::error('Send order mail failed (VNPAY): ' . $e->getMessage());
                        }
                    });
                } else {
                    \Log::warning('Skip order mail (VNPAY) due to invalid/empty email', ['email' => $recipientEmail, 'order_id' => $order->id]);
                }
                // ====== HẾT PHẦN THÊM ======
    ...
    //         $email = $request->input('ship_to_different')
    //             ? $request->input('shipping_email')
    //             : $request->input('email');

    //         if (empty($email)) {
    //             return back()->with('error', 'Vui lòng nhập email để nhận thông tin đơn hàng.');
    //         }
    ...
    //         return redirect()->route('order.success')->with('message', 'Đặt hàng thành công!');
    //     });
    // }
}
