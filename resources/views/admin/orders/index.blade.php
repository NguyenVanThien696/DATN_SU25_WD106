                        ...
                        <div class="text-center text-muted py-5">
                            ...
                            <h5 class="fw-semibold">
                             @php
                                ...
                                try {
                                    if (preg_match('/^\d{2}\/\d{2}\/\d{4}$/', $queryDate)) {
                                        $parsedDate = \Carbon\Carbon::createFromFormat('d/m/Y', $queryDate)->toDateString();
                                    } else {
                                        $parsedDate = \Carbon\Carbon::parse($queryDate)->toDateString();
                                    }
                                } catch (\Exception $e) {
                                    $parsedDate = null;
                                }
                                    ...
                                    @foreach ($orders as $order)
                                        ...
                                        <tr>
                                            ...
                                            <td class="customer-name"
                                                title="{{ $order->shippingAddress->name ?? $order->user->name }}">
                                                {{ Str::limit($order->shippingAddress->name ?? $order->user->name, 18) }}
                                            </td>
                                            <td>
                                                @if ($firstItem?->productVariant?->product?->image)
                                                    <img src="{{ asset('storage/' . $firstItem->productVariant->product->image) }}" alt
                                                        class="order-img">
                                                @else
                                            ...
                                            <td class="text-start" title="{{ $firstItem->productVariant->product->name ?? '---' }}">
                                                <strong>{{ Str::limit($firstItem->productVariant->product->name ?? '---', 30) }}</strong>
                                                x {{ $firstItem->quantity }}<br>
                                            ...
                                            <td>
                                                @if (empty($availableTransitions))
                                                    <span
                                                        class="badge {{ $statusClass }}">{{ $statusList[$order->status] ?? '---' }}</span>
                                                @else
                                                    <div class="dropdown">
                                                                ...
                                                                <li>
                                                                    <form action="{{ route('admin.order.updateStatus', $order->id) }}"
                                                                        ...
                                                                        <button class="dropdown-item"
                                                                            type="submit">{{ $statusList[$key] }}</button>
                                                                    </form>
                                                                </li>
                                                            @endforeach
                                                        </ul>
                                                ...
                                                <div class="d-flex justify-content-center align-items-center gap-1 flex-wrap">
                                                    ...
                                                    @if ($order->status === 'cancelled_paid')
                                                        <form action="{{ route('admin.order.refund', $order->id) }}" method="POST"
                                                            onsubmit="return confirm('Xác nhận hoàn tiền?')" class="d-inline">
                                                            @csrf
