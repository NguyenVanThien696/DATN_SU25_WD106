                            ...
                            <table class="table table-striped table-hover text-nowrap"
                                ...
                                <thead>
                                    <tr style="background-color: #f8f9fa;">
                                        ...
                                        <th style="text-align: center; padding: 10px; border: 1px solid #dee2e6;">Tiêu đề</th>
                                        <th style="text-align: center; padding: 10px; border: 1px solid #dee2e6;">Ảnh</th>
                                        <th style="text-align: center; padding: 10px; border: 1px solid #dee2e6;">Vị trí</th>
                                        <th style="text-align: center; padding: 10px; border: 1px solid #dee2e6;">Trạng thái</th>
                                        <th style="text-align: center; padding: 10px; border: 1px solid #dee2e6;">Thao tác</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($banners as $banner)
                                        <tr>
                                            ...
                                            <td style="text-align: center; padding: 10px; border: 1px solid #dee2e6;">
                                                @if ($banner->image)
                                                    <img src="{{ asset('storage/' . $banner->image) }}" style="width: 100px; height: auto; border-radius: 5px;">
                                                @else
                                            ...
                                            <td style="text-align: center; padding: 10px; border: 1px solid #dee2e6;">
                                                <a href="{{ route('admin.banners.edit', $banner->id) }}" class="btn btn-sm btn-outline-warning">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route('admin.banners.destroy', $banner->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa banner này?')">
